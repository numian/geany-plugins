...
static gboolean project_filter_web_php(const gchar *file)
{
    if (filetypes_detect_from_file(file)->id == GEANY_FILETYPES_PHP || 
        filetypes_detect_from_file(file)->id == GEANY_FILETYPES_HTML || 
        filetypes_detect_from_file(file)->id == GEANY_FILETYPES_JS || 
        filetypes_detect_from_file(file)->id == GEANY_FILETYPES_CSS || 
        filetypes_detect_from_file(file)->id == GEANY_FILETYPES_PO)
        return TRUE;
    return FALSE;
}
